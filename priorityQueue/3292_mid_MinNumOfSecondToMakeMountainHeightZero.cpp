...
//     * 2 = 3 seconds. Worker 2 reduces the height by 1, taking workerTimes[2]
//     = 1 second.
//
...
//     * 2 = 9 seconds. Worker 1 reduces the height by 3, taking workerTimes[1]
//     + workerTimes[1] * 2 + workerTimes[1] * 3 = 12 seconds. Worker 2 reduces
//     the height by 3, taking workerTimes[2] + workerTimes[2] * 2 +
//     workerTimes[2] * 3 = 12 seconds. Worker 3 reduces the height by 2, taking
...
public:
  long long minNumberOfSeconds(int mountainHeight, vector<int> &workerTimes) {
    ...
    for (int i = 0; i < workerTimes.size(); i++) {
      pq.push({workerTimes[i], {2, workerTimes[i]}});
    }
    for (int i = 0; i < mountainHeight; i++) {
      pair<long long, pair<int, int>> temp = pq.top();
      pq.pop();
      pq.push({temp.first + 1LL * temp.second.first * temp.second.second,
               {temp.second.first + 1, temp.second.second}});
    }
    long long ans = 0;
    while (!pq.empty()) {
      pair<long long, pair<int, int>> temp = pq.top();
      cout << temp.first << " | " << temp.second.first << " "
           << temp.second.second << endl;
      pq.pop();
      if (temp.second.first != 2) {
        ans = max(ans, (temp.first -
                        1LL * (temp.second.first - 1) * (temp.second.second)));
      }
    }
    return ans;
  }
