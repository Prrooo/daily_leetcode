...
// the smallest index. For example, if costs = [3,2,7,7,1,2] and candidates = 2,
// then in the first hiring session, we will choose the 4th worker because they
// have the lowest cost [3,2,7,7,1,2]. In the second hiring session, we will
// choose 1st worker because they have the same lowest cost as 4th worker but
// they have the smallest index [3,2,7,7,2]. Please note that the indexing may
// be changed in the process. If there are fewer than candidates workers
// remaining, choose the worker with the lowest cost among them. Break the tie
// by the smallest index. A worker can only be chosen once. Return the total
// cost to hire exactly k workers.
...
long long totalCost(vector<int> &costs, int k, int candidates) {
  ...
  while (index < k && !first.empty()) {
    ans += first.top();
    first.pop();
    index++;
  }
  while (index < k && !last.empty()) {
    ans += last.top();
    last.pop();
    index++;
  }
