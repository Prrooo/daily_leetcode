...
public:
  ...
  bool solve(vector<vector<char>> &grid, vector<vector<bool>> &vis, int o_i,
             int o_j, int n_i, int n_j, int n, int m) {
    if (vis[n_i][n_j])
      return true;
    vis[n_i][n_j] = true;
    for (auto i : dir) {
      int tempi = n_i + i.first;
      int tempj = n_j + i.second;
      if (tempi < n && tempi >= 0 && tempj < m && tempj >= 0 &&
          (tempi != o_i || tempj != o_j)) {
        if (grid[o_i][o_j] == grid[tempi][tempj] &&
            solve(grid, vis, n_i, n_j, tempi, tempj, n, m)) {
          return true;
        }
      }
    }
