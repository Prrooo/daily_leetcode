...
class Solution {
public:
  int solve(vector<int> &energy, int start, int n, int k, vector<int> &dp) {
    if (start >= n)
      return 0;
    if (dp[start] != -1)
      return dp[start];
    return dp[start] = energy[start] + solve(energy, start + k, n, k, dp);
  }
  ...
  int maximumEnergy(vector<int> &energy, int k) {
    ...
    vector<int> dp(n, -1);
    for (int i = 0; i < n; i++) {
      ans = max(ans, solve(energy, i, n, k, dp));
    }
    return ans;
  }
};
