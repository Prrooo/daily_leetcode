...
// cannot jump to index 5 because 13 > 9. You cannot jump to index 4 because
// index 5 is between index 4 and 6 and 13 > 9. Similarly You cannot jump from
// index 3 to index 2 or index 1.
//
// Example 2:
//
...
class Solution {
public:
  int solve(vector<int> &arr, vector<int> &dp, int d, int index, int n) {
    int left = max(0, index - d), right = min(n - 1, index + d);
    int ans = 0;
    for (int i = index - 1; i >= left; i--) {
      ...
      if (dp[i] == -1) {
        ans = max(ans, 1 + solve(arr, dp, d, i, n));
      } else {
        ans = max(ans, 1 + dp[i]);
      }
    }
