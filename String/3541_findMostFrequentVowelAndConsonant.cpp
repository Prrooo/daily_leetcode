...
//     frequency. Find the consonant (all other letters excluding vowels) with
...
// string, consider their frequency as 0. The frequency of a letter x is the
...
//     frequency is 2. The consonants are: 's' (frequency 4), 'c' (frequency 2).
//     The maximum frequency is 4. The output is 2 + 4 = 6.
//
// Example 2:
//
// Input: s = "aeiaeia"
...
//     The maximum frequency is 3. There are no consonants in s. Hence, maximum
...
int maxFreqSum(string s) {
  int n = s.size(), vMax = 0, cMax = 0;
  vector<int> count(26, 0);
  for (int i = 0; i < n; i++) {
    count[s[i] - 'a']++;
  }
  for (int i = 0; i <= 26; i++) {
    if (i + 'a' == 'a' || i + 'a' == 'e' || i + 'a' == 'i' || i + 'a' == 'o' ||
        i + 'a' == 'u') {
      vMax = max(vMax, count[i]);
    } else {
