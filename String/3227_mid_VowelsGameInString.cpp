...
//     which contains 0 vowels. The resulting string is s = "er". Alice plays
//     third, she can delete the whole string s = "er" which contains 1 vowel.
...
class Solution {
public:
  bool solve(string s, bool first, int start) {
    int count = 0, j = -1;
    if (first) {
    ...
    if (j == -1)
      return true;
    return solve(s, true, j);
  }

  bool doesAliceWin(string s) { return solve(s, true, 0); }
};
