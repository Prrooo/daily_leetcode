...
// Find the number of
//
// of [0, 1, 2, ..., (n - 1)] that represent a valid order in which the
// computers can be unlocked, starting from computer 0 as the only initially
// unlocked one.
//
...
//         complexity[1]. Unlock computer 2 with password of computer 1 since
//         complexity[1] < complexity[2].
//     [0, 2, 1]
//         Unlock computer 0 first with root password.
//         Unlock computer 2 with password of computer 0 since complexity[0] <
//         complexity[2]. Unlock computer 1 with password of computer 0 since
...
public:
  int countPermutations(vector<int> &complexity) {
    int minEle = *min_element(complexity.begin() + 1, complexity.end()),
        n = complexity.size();
    if (complexity[0] >= minEle)
      return 0;
    int MOD = 1e9 + 7;
    int ans = 1;
    for (int i = 2; i < n; i++) {
